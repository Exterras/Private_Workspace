...
{
    if (c == '\n') put_ch('\r');
    while( !((*(volatile unsigned long *) 0x40004400) & 0x80) );
    *(volatile unsigned long *) 0x40004404 = c;
}

char get_byte(void)
{
    while( !((*(volatile unsigned long *) 0x40004400) & 0x20) ) ;
    return (char) (*(volatile unsigned long *) 0x40004404 & 0xff);
}
